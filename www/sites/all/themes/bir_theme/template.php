...
function bir_theme_preprocess_browse(&$variables) {
  if ($variables['view']->current_display == 'browse_map' || $variables['view']->current_display == 'browse_map_all') {
    // When not looking at localiy or specimen, need to replace 
    // field values created for Image to Specimen relationship
    ...
    foreach ($pre_locations as $location_key => $fields) {
      ...
      while (list($key, $value) = each($fields)) {
        if (in_array($key, $replace_fields)  && !empty($fields[$key.'_1'])) {
          $value = $fields[$key . '_1'];
          unset($fields[$key . '_1']);
        }
...
function bir_theme_preprocess_node(&$variables) {
  if ($variables['is_front']) {
    unset($variables['content']['links']);
  }
}

function bir_theme_date_combo($variables) {
