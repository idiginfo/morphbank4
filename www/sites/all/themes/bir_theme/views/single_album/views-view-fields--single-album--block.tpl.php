...
$type = str_replace("bir ", "", strtolower($fields['type']->content));
...
switch ($type) {
  case 'image':
    ...
    break;
  default:
    $node_thumb = "/bir-api/node/{$fields['standard_image']->content}.thumb";
    $node_jpeg = "/bir-api/node/{$fields['standard_image']->content}.jpeg";
    $full = !empty($fields['standard_image']->content) ? $node_thumb : $default_img;
    $full2 = !empty($fields['standard_image']->content) ? $node_jpeg : $default_img;
    $path = "/$type/{$fields['nid']->content}";
}
?>
